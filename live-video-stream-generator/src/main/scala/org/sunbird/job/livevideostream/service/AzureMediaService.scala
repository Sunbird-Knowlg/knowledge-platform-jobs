...
import org.sunbird.job.livevideostream.helpers.{AzureRequestBody, MediaResponse, Response}
...
abstract class AzureMediaService extends IMediaService {
  ...
  private def getToken()(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): String = {
  ...
  protected def getJobDetails(jobId: String)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): MediaResponse = {
  ...
  protected def createAsset(assetId: String, jobId: String)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): MediaResponse = {
  ...
  protected def createStreamingLocator(streamingLocatorName: String, assetName: String)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): MediaResponse = {
  ...
  protected def getStreamingLocator(streamingLocatorName: String)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): MediaResponse = {
    val url = getApiUrl("stream_locator").replace("streamingLocatorName", streamingLocatorName)
    val response:MediaResponse = Response.getResponse(httpUtil.get(url, getDefaultHeader()))
    if(response.responseCode == "OK"){
      response
  ...
  protected def getStreamUrls(streamingLocatorName: String)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): MediaResponse = {
  ...
  protected def getApiUrl(apiName: String)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): String = {
  ...
  protected def getDefaultHeader()(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): Map[String, String] = {
  ...
  protected def prepareStreamingUrl(streamLocatorName: String, jobId: String)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): Map[String, AnyRef] = {
