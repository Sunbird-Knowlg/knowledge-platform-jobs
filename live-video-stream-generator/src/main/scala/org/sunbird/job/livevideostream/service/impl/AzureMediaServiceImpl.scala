...
import org.sunbird.job.livevideostream.service.AzureMediaService
import org.sunbird.job.livevideostream.task.LiveVideoStreamGeneratorConfig
import org.sunbird.job.util.HttpUtil
import org.sunbird.job.livevideostream.helpers.{AzureRequestBody, AzureResult, MediaRequest, MediaResponse, Response}
...
object AzureMediaServiceImpl extends AzureMediaService {
  ...
  override def submitJob(request: MediaRequest)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): MediaResponse = {
  ...
  override def getJob(jobId: String)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): MediaResponse = {
  ...
  override def getStreamingPaths(jobId: String)(implicit config: LiveVideoStreamGeneratorConfig, httpUtil: HttpUtil): MediaResponse = {
