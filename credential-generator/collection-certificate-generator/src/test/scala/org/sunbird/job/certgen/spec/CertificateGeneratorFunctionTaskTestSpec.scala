...
class CertificateGeneratorFunctionTaskTestSpec extends BaseTestSpec {
  ...
  override protected def beforeAll(): Unit = {
    ...
    when(mockHttpUtil.post(endsWith("/private/v1/user/feed/create"), any[String], any[Map[String, String]]())).thenReturn(HTTPResponse(200, """{"id":"api.user.feed.create","ver":"v1","ts":"2020-10-30 13:20:54:940+0000","params":{"resmsgid":null,"msgid":"518d3404-cf1f-4001-81a5-0c58647b32fe","err":null,"status":"success","errmsg":null},"responseCode":"OK","result":{"response":"SUCCESS"}}"""))
  ...
  ignore should "generate certificate and add to the registry" in {
    ...
    intercept[JobExecutionException](new CertificateGeneratorStreamTask(jobConfig, notifierConfig, userFeedConfig, mockKafkaUtil, mockHttpUtil, storageService).process())
