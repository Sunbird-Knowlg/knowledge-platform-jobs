...
class CertificateUserUtilSpec extends BaseTestSpec {
  ...
  it should "throw exception with empty criteria for getUserIdsBasedOnCriteria" in intercept[Exception] {
    mockAll()
    val template = gson.fromJson(EventFixture.USER_UTIL_TEMPLATE_2, classOf[java.util.Map[String, AnyRef]]).asScala.toMap
    val edata = new Event(new java.util.HashMap[String, Any]() {{ put("edata", gson.fromJson(EventFixture.USER_UTIL_EDATA, classOf[java.util.Map[String, AnyRef]]))}})
  ...
  it should "throw exception with invalid criteria for getUserIdsBasedOnCriteria" in intercept[Exception] {
    mockAll()
    val template = gson.fromJson(EventFixture.USER_UTIL_TEMPLATE_3, classOf[java.util.Map[String, AnyRef]]).asScala.toMap
    val edata = new Event(new java.util.HashMap[String, Any]() {{ put("edata", gson.fromJson(EventFixture.USER_UTIL_EDATA, classOf[java.util.Map[String, AnyRef]]))}})
    CertificateUserUtil.getUserIdsBasedOnCriteria(template, edata)(metrics, cassandraUtil, jobConfig)
  }
