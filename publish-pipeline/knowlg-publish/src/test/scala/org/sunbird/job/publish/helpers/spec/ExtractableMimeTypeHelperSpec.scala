...
import org.sunbird.job.knowlg.publish.helpers.ExtractableMimeTypeHelper
...
class ExtractableMimeTypeHelperSpec extends FlatSpec with BeforeAndAfterAll with Matchers with MockitoSugar {
  ...
  "getCloudStoreURL" should "return extraction path with scorm for SCORM_Archive" in {
    val obj: ObjectData = new ObjectData("do_123", Map[String, AnyRef]("identifier" -> "do_123", "mimeType" -> "application/vnd.ekstep.scorm-archive"))
    val result = ExtractableMimeTypeHelper.getCloudStoreURL(obj, cloudStorageUtil, jobConfig)
    result should include ("scorm")
  }
