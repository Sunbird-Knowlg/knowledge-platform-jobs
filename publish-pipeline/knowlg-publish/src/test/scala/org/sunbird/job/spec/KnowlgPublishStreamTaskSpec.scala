...
import org.sunbird.job.knowlg.function.{CollectionPublishFunction, QuestionSetPublishFunction}
...
class KnowlgPublishStreamTaskSpec extends BaseTestSpec {
  ...
  def initialize(): Unit = {
    when(mockKafkaUtil.kafkaJobRequestSource[Event](jobConfig.kafkaInputTopic)).thenReturn(null.asInstanceOf[KafkaSource[Event]])
  }
