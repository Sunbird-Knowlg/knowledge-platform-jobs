...
class KnowlgPublishStreamTask(config: KnowlgPublishConfig, kafkaConnector: FlinkKafkaConnector, httpUtil: HttpUtil) {
  ...
  def process(): Unit = {
    ...
    contentPublish.getSideOutput(config.generateVideoStreamingOutTag).sinkTo(kafkaConnector.kafkaStringSinkV2(config.postPublishTopic))
    contentPublish.getSideOutput(config.mvcProcessorTag).sinkTo(kafkaConnector.kafkaStringSinkV2(config.mvcTopic))
    contentPublish.getSideOutput(config.contentMetadataEventOutTag).sinkTo(kafkaConnector.kafkaStringSinkV2(config.contentMetadataTopic))
   ...
   val collectionPublish = processStreamTask.getSideOutput(config.collectionPublishOutTag).process(new CollectionPublishFunction(config, httpUtil))
    ...
    collectionPublish.getSideOutput(config.generatePostPublishProcessTag).sinkTo(kafkaConnector.kafkaStringSinkV2(config.postPublishTopic))
    ...
    questionPublish.getSideOutput(config.failedEventOutTag).sinkTo(kafkaConnector.kafkaStringSinkV2(config.kafkaErrorTopic))
    ...
    questionSetPublish.getSideOutput(config.failedEventOutTag).sinkTo(kafkaConnector.kafkaStringSinkV2(config.kafkaErrorTopic))
