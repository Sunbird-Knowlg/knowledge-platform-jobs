		...
		if (event.validEvent()) {
			...
			event.objectType match {
				case "Question" | "QuestionImage" => {
					...
					context.output(config.questionPublishOutTag, PublishMetadata(event.objectId, event.objectType, event.mimeType, event.pkgVersion, event.publishType,event.publishChainMetadata))
				}
				case "QuestionSet" | "QuestionSetImage" => {
					...
					context.output(config.questionSetPublishOutTag, PublishMetadata(event.objectId, event.objectType, event.mimeType, event.pkgVersion, event.publishType,event.publishChainMetadata))
				}
				case _ => {
