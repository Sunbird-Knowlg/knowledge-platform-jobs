...
class ThumbnailGeneratorSpec extends FlatSpec with BeforeAndAfterAll with Matchers {
  ...
  "Object Thumbnail Generator generateThumbnail" should "add the thumbnail to ObjectData" in {
    ...
    val metadata = Map("identifier" -> "do_123", "appIcon" -> "https://staging.sunbirded.org/content/preview/assets/icons/avatar_anonymous.png", "IL_UNIQUE_ID" -> "do_123", "objectType" -> "QuestionSet", "name" -> "Test QuestionSet", "status" -> "Live")
    ...
    resultMetadata.getOrElse("posterImage", "").asInstanceOf[String] shouldBe "https://staging.sunbirded.org/content/preview/assets/icons/avatar_anonymous.png"
