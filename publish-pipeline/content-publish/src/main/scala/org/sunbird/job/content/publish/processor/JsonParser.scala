...
object JsonParser {
    ...
    def getManifest(jsonObject: Map[String, AnyRef], validateMedia: Boolean): Manifest = {
        if(null != jsonObject && jsonObject.keySet.contains("manifest") && jsonObject.get("manifest").get.isInstanceOf[List[Map[String, AnyRef]]]) throw new Exception("Error! JSON Object is Expected for the Element. manifest")
    ...
    def validateController(obj: Map[String, AnyRef]) = {
        ...
        if(null == id || StringUtils.isBlank(id.toString))
            throw new Exception("Error! Invalid Controller ('id' is required.)")
        if(null == `type` || StringUtils.isBlank(`type`))
            throw new Exception("Error! Invalid Controller ('type' is required.)")
        if(!"items".equalsIgnoreCase(`type`) && !"data".equalsIgnoreCase(`type`))
            throw new Exception("Error! Invalid Controller ('type' should be either 'items' or 'data')")
    }
    ...
    def getMedia(mediaJson: Map[String, AnyRef], validateMedia: Boolean): Media = {
        if(null != mediaJson) {
            ...
            if(StringUtils.isBlank(id))
                throw new Exception("Error! Invalid Media ('id' is required.)")
            if(!(StringUtils.isNotBlank(`type`) &&(`type`.equalsIgnoreCase("js") || `type`.equalsIgnoreCase("css"))))
                throw new Exception("Error! Invalid Media ('type' is required.)")
            if(StringUtils.isBlank(src))
                throw new Exception("Error! Invalid Media ('src' is required.)")
