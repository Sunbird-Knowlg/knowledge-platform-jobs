...
import org.sunbird.job.content.publish.helpers.{CollectionPublisher, ContentPublisher}
...
class CollectionPublisherSpec extends FlatSpec with BeforeAndAfterAll with Matchers with MockitoSugar {
  ...
  "getObjectWithEcar" should "return object with ecar url" in {
    ...
    val result = new TestCollectionPublisher().getObjectWithEcar(data, List(EcarPackageType.FULL.toString, EcarPackageType.ONLINE.toString))(ec, cloudStorageUtil, jobConfig, defCache, defConfig, httpUtil)
