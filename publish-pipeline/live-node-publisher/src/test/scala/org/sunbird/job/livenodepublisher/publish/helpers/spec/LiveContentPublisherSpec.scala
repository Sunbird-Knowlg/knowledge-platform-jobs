...
class LiveContentPublisherSpec extends FlatSpec with BeforeAndAfterAll with Matchers with MockitoSugar {
  ...
  "saveExternalData " should "save external data to cassandra table" in {
    ...
    new TestLiveContentPublisher().saveExternalData(data, readerConfig)
  }
  ...
  "getHierarchy " should "do nothing " in {
    ...
    new TestLiveContentPublisher().getHierarchy(identifier, readerConfig)
  }

  "getExtDatas " should "do nothing " in {
    val identifier = "do_11329603741667328018"
    new TestLiveContentPublisher().getExtDatas(List(identifier), readerConfig)
  }

  "getHierarchies " should "do nothing " in {
    val identifier = "do_11329603741667328018"
    new TestLiveContentPublisher().getHierarchies(List(identifier), readerConfig)
  }
  ...
  "getObjectWithEcar" should "return object with ecar url" in {
    ...
    val result = new TestLiveContentPublisher().getObjectWithEcar(data, List(EcarPackageType.FULL.toString, EcarPackageType.ONLINE))(ec, mockJanusGraphUtil, cloudStorageUtil, jobConfig, defCache, defConfig, httpUtil)
