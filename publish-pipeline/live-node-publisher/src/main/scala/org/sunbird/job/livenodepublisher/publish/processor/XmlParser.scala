...
object XmlParser {
  ...
  def processDocument(root: Node): Plugin = {
    if (null != root) {
      Plugin(getId(root), getData(root), "", getCdata(root), getChildrenPlugin(root), getManifest(root, validateNode = true), getControllers(root \ "controllers"), getEvents(root))
  ...
  def getCdata(node: Node): String = {
    if (null != node && node.child.nonEmpty) {
  ...
  def getChildrenPlugin(node: Node): List[Plugin] = {
    if (null != node && node.child.nonEmpty) {
      ...
      nodeList.toList.filter(childNode => childNode.isInstanceOf[Elem] && !nonPluginElements.contains(childNode.label) && !"event".equalsIgnoreCase(childNode.label))
        .map(chilNode => Plugin(getId(chilNode), getData(chilNode), getInnerText(chilNode), getCdata(chilNode), getChildrenPlugin(chilNode), getManifest(chilNode, validateNode = false), getControllers(chilNode \ "controllers"), getEvents(chilNode)))
    } else {
      List()
    }
  }
  ...
  def getInnerText(node: Node): String = {
    if (null != node && node.isInstanceOf[Elem] && node.child.nonEmpty) {
      val childNodes = node.child
      val innerTextlist = childNodes.toList.filter(childNode => childNode.isInstanceOf[Text]).map(item => item.text)
      if (innerTextlist.nonEmpty) innerTextlist.head else ""
    } else ""
  }
    ...
    if (null != node) {
      ...
      if (validateNode) {
        if (StringUtils.isBlank(id) && !(StringUtils.isNotBlank(`type`) && (StringUtils.equalsIgnoreCase(`type`, "js") || StringUtils.equalsIgnoreCase(`type`, "css"))))
          throw new InvalidInputException("Error! Invalid Media ('id' is required.) in '" + node.buildString(true) + "' ...")
        if (StringUtils.isBlank(`type`))
          throw new InvalidInputException("Error! Invalid Media ('type' is required.) in '" + node.buildString(true) + "' ...")
        if (StringUtils.isBlank(src))
          throw new InvalidInputException("Error! Invalid Media ('src' is required.) in '" + node.buildString(true) + "' ...")
      }
  ...
  def getManifest(node: Node, validateNode: Boolean): Manifest = {
    ...
    val mediaList = {
      if (null != manifestNode && manifestNode.child.nonEmpty) {
  ...
  def getElementXml(data: Map[String, AnyRef]): StringBuilder = {
    val strBuilder = StringBuilder.newBuilder
    if (null != data) {
      strBuilder.append(START_TAG_OPENING + data("cwp_element_name"))
  ...
  def getPluginsXml(childrenPlugin: List[Plugin]): StringBuilder = {
    val strBuilder = StringBuilder.newBuilder
    if (null != childrenPlugin && childrenPlugin.nonEmpty) {
