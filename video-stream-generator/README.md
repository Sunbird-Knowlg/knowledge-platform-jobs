...
Video Stream Generator job is to generate the stream url for uploaded video (mp4/webm) files
...
1. Download flink-1.15.2-scala_2.12 from [apache-flink-downloads](https://www.apache.org/dyn/closer.lua/flink/flink-1.15.2/flink-1.15.2-bin-scala_2.12.tgz). 
...
```
./bin/flink run -m yarn-cluster -p 2 -yjm 1024m -ytm 1024m <knowledge-platform-jobs>/video-stream-generator/target/video-stream-generator-0.0.1.jar
```
...
./bin/flink run -m localhost:8081 <knowledge-platform-job>/video-stream-generator/target/video-stream-generator-0.0.1.jar
