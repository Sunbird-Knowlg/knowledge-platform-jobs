...
trait CompositeSearchIndexerHelper {
  ...
  private[this] val logger =
    LoggerFactory.getLogger(classOf[CompositeSearchIndexerHelper])

  def createCompositeSearchIndex()(esUtil: ElasticSearchUtil): Boolean = {
    val settings =
      """{"max_ngram_diff":"29","mapping":{"total_fields":{"limit":"1500"}},"analysis":{"filter":{"mynGram":{"token_chars":["letter","digit","whitespace","punctuation","symbol"],"min_gram":"1","type":"nGram","max_gram":"30"}},"analyzer":{"cs_index_analyzer":{"filter":["lowercase","mynGram"],"type":"custom","tokenizer":"standard"},"keylower":{"filter":"lowercase","tokenizer":"keyword"},"cs_search_analyzer":{"filter":["lowercase"],"type":"custom","tokenizer":"standard"}}}}"""
    val mappings =
  ...
  private def getIndexDocument(
      identifier: String
  )(esUtil: ElasticSearchUtil): scala.collection.mutable.Map[String, AnyRef] = {
    val documentJson: String = esUtil.getDocumentAsString(identifier)
    val indexDocument =
      if (documentJson != null && documentJson.nonEmpty)
        ScalaJsonUtil.deserialize[scala.collection.mutable.Map[String, AnyRef]](
          documentJson
        )
      else scala.collection.mutable.Map[String, AnyRef]()
    indexDocument
  }
  ...
  def getIndexDocument(
  ...
  )(esUtil: ElasticSearchUtil): Map[String, AnyRef] = {
    val identifier = message.getOrElse("nodeUniqueId", "").asInstanceOf[String]
    val indexDocument =
      if (isUpdate) getIndexDocument(identifier)(esUtil)
      else scala.collection.mutable.Map[String, AnyRef]()
    val transactionData = message
      .getOrElse("transactionData", Map[String, Any]())
      .asInstanceOf[Map[String, Any]]

    if (transactionData.nonEmpty) {
      val addedProperties = transactionData
        .getOrElse("properties", Map[String, AnyRef]())
      ...
      addedProperties.foreach(property => {
        if (!definition.externalProperties.contains(property._1)) {
          ...
          if (propertyNewValue == null) indexDocument.remove(property._1)
          else
            indexDocument.put(
              property._1,
              addMetadataToDocument(property._1, propertyNewValue, nestedFields)
            )
        }
      })

      val addedRelations = transactionData
      ...
      if (addedRelations.nonEmpty) {
        addedRelations.foreach(rel => {
          ...
          val title =
            definition.relationLabel(targetObjType, direction, relationType)
          if (title.nonEmpty) {
            val list = indexDocument
              .getOrElse(title.get, List[String]())
              .asInstanceOf[List[String]]
            val id = rel.getOrElse("id", "").asInstanceOf[String]
            if (!list.contains(id))
              indexDocument.put(title.get, (id :: list).asInstanceOf[AnyRef])
          }
        })
      }

      val removedRelations = transactionData
    ...
    indexDocument.put(
      "graph_id",
      message.getOrElse("graphId", "").asInstanceOf[String]
    )
    indexDocument.put(
      "node_id",
    ...
    indexDocument.put(
      "identifier",
      message.getOrElse("nodeUniqueId", "").asInstanceOf[String]
    )
    indexDocument.put(
      "objectType",
      message.getOrElse("objectType", "").asInstanceOf[String]
    )
    indexDocument.put(
      "nodeType",
      message.getOrElse("nodeType", "").asInstanceOf[String]
    )
    indexDocument.toMap
  }

  def upsertDocument(identifier: String, jsonIndexDocument: String)(
      esUtil: ElasticSearchUtil
  ): Unit = {
    esUtil.addDocument(identifier, jsonIndexDocument)
  }

  def processESMessage(
      compositeObject: CompositeIndexer
  )(esUtil: ElasticSearchUtil, defCache: DefinitionCache): Unit = {
    val definition = defCache.getDefinition(
      compositeObject.objectType,
      compositeObject.getVersionAsString(),
      compositeObject.getDefinitionBasePath()
    )

    val compositeMap = compositeObject.message.asScala.toMap
    upsertDocument(
      compositeObject.identifier,
      compositeMap,
      definition,
      compositeObject.getNestedFields(),
      compositeObject.getIgnoredFields()
    )(esUtil)
  }
  ...
  private def upsertDocument(
      ...
      nestedFields: List[String],
      ignoredFields: List[String]
  )(esUtil: ElasticSearchUtil): Unit = {
    val operationType =
      message.getOrElse("operationType", "").asInstanceOf[String]
    operationType match {
      case "CREATE" =>
        val indexDocument = getIndexDocument(
          message,
          false,
          definition,
          nestedFields,
      ...
      case "DELETE" =>
        ...
        val visibility =
          indexDocument.getOrElse("visibility", "").asInstanceOf[String]
        if (StringUtils.equalsIgnoreCase("Parent", visibility))
          logger.info(
            s"Not deleting the document (visibility: Parent) with ID: $id"
          )
        else esUtil.deleteDocument(identifier)
      case _ =>
        logger.info(
          s"Unknown Operation Type : $operationType for the identifier: $identifier."
        )
    }
  }
