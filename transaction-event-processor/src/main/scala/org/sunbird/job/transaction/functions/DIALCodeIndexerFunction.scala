...
import org.sunbird.job.transaction.compositesearch.helpers.DIALCodeIndexerHelper
...
class DIALCodeIndexerFunction(
  ...
  override def processElement(
    ...
    try {
      upsertDIALCodeDocument(event.id, event.getMap().asScala.toMap)(
        elasticUtil
      )
  ...
  override def metricsList(): List[String] = {
    List(
      config.successDialcodeExternalEventCount,
      config.failedDialcodeExternalEventCount,
      config.dialcodeExternalEventCount
    )
  }
}
