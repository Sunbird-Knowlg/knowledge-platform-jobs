...
import org.sunbird.job.transaction.domain.{
  AuditHistoryRecord,
  Event,
  ObsrvEvent
}
...
trait TransactionEventProcessorService {
  private[this] lazy val logger =
    LoggerFactory.getLogger(classOf[TransactionEventProcessorService])
  ...
  private val systemPropsList = List(
    "IL_SYS_NODE_TYPE",
    "IL_FUNC_OBJECT_TYPE",
    "IL_UNIQUE_ID",
    "IL_TAG_NAME",
  ...
  def processAuditEvent(
      message: Event,
      context: ProcessFunction[Event, String]#Context,
      metrics: Metrics
  )(implicit config: TransactionEventProcessorConfig): Unit = {
    logger.info("AUDIT Event::" + JSONUtil.serialize(message))
    logger.info(
      "Input Message Received for : [" + message.nodeUniqueId + "], Txn Event createdOn:" + message.createdOn + ", Operation Type:" + message.operationType
    )
    try {
      val (auditEventStr, objectType) =
        getAuditMessage(message)(config, metrics)
      if (StringUtils.isNotBlank(objectType)) {
        context.output(config.auditOutputTag, auditEventStr)
        logger.info(
          "Telemetry Audit Message Successfully Sent for : " + message.objectId + " :: event ::" + auditEventStr
        )
        metrics.incCounter(config.auditEventSuccessCount)
      } else {
        logger.info(
          "Skipped event as the objectype is not available, event =" + auditEventStr
        )
      ...
      case e: Exception =>
        logger.error(
          "Failed to process message :: " + JSONUtil.serialize(message),
          e
        )
        throw e
    }
  ...
  def processEvent(
    ...
    try {
      ...
      if (StringUtils.isNotBlank(objectType)) {
        ...
        val obsrvEvent =
          new ObsrvEvent(message.getMap(), message.partition, message.offset)
        ...
        logger.info(
          "Telemetry Audit Message Successfully Sent for : " + message.objectId + " :: event ::" + eventStr
        )
  ...
  def getDefinition(objectType: String)(implicit
      config: TransactionEventProcessorConfig,
      metrics: Metrics
  ): ObjectDefinition = {
    try {
      definitionCache.getDefinition(
        objectType,
  ...
  def getAuditMessage(message: Event)(implicit
      config: TransactionEventProcessorConfig,
      metrics: Metrics
  ): (String, String) = {
    ...
    val env =
      if (null != objectType) objectType.toLowerCase.replace("image", "")
      else "system"

    val definitionNode: ObjectDefinition = getDefinition(objectType)

    val propertyMap =
      message.transactionData("properties").asInstanceOf[Map[String, AnyRef]]
    val statusMap =
      propertyMap.getOrElse("status", null).asInstanceOf[Map[String, AnyRef]]
    val lastStatusChangedOn = propertyMap
      .getOrElse("lastStatusChangedOn", null)
      .asInstanceOf[Map[String, AnyRef]]
    val addedRelations = message.transactionData
      .getOrElse("addedRelations", List[Map[String, AnyRef]]())
      .asInstanceOf[List[Map[String, AnyRef]]]
    val removedRelations = message.transactionData
      .getOrElse("removedRelations", List[Map[String, AnyRef]]())
      .asInstanceOf[List[Map[String, AnyRef]]]
    ...
    var currStatus = ""
    var duration = ""
    val pkgVerMap = propertyMap
      .getOrElse("pkgVersion", null)
      .asInstanceOf[Map[String, AnyRef]]
    ...
    if (null != statusMap) {
      ...
      if (
        StringUtils.isNotBlank(currStatus) && StringUtils.isNotBlank(
          prevStatus
        ) && null != lastStatusChangedOn
      ) {
    ...
    val propsExceptSystemProps =
      props.filter(prop => !systemPropsList.contains(prop))
    val cdata = getCData(addedRelations, removedRelations, propertyMap)

    var context: Map[String, String] =
      getContext(message.channelId(config.defaultChannel), env)

    objectType =
      if (null != objectType)
        objectType.replaceAll(OBJECT_TYPE_IMAGE_SUFFIX, "")
      else objectType
    context ++= Map("objectId" -> message.objectId, "objectType" -> objectType)

    if (StringUtils.isNotBlank(duration))
      context ++= Map("duration" -> duration)
    if (StringUtils.isNotBlank(pkgVersion))
      context ++= Map("pkgVersion" -> pkgVersion)
    if (StringUtils.isNotBlank(message.userId))
      context ++= Map(TelemetryParams.ACTOR.name -> message.userId)

    if (propsExceptSystemProps.nonEmpty) {
      val cdataList = gson.fromJson(
        JSONUtil.serialize(cdata),
      ...
      auditMap = TelemetryGenerator.audit(
        JSONUtil
          .deserialize[util.Map[String, String]](JSONUtil.serialize(context)),
        JSONUtil.deserialize[util.List[String]](
          JSONUtil.serialize(propsExceptSystemProps)
        ),
        currStatus,
        prevStatus,
        cdataList
      ...
      logger.info(
        "Audit Message for Content Id [" + message.objectId + "] : " + auditMap
      );
  ...
  /** @param addedRelations
    ...
    * @return
    */
  private def getCData(
      addedRelations: List[Map[String, AnyRef]],
      removedRelations: List[Map[String, AnyRef]],
      propertyMap: Map[String, AnyRef]
  ): List[Map[String, AnyRef]] = {
    var cdata = List[Map[String, AnyRef]]()
    if (
      null != propertyMap && propertyMap.nonEmpty && propertyMap.contains(
        "dialcodes"
      )
    ) {
      val dialcodeMap =
        propertyMap("dialcodes").asInstanceOf[Map[String, AnyRef]]
      val dialcodes = dialcodeMap("nv").asInstanceOf[List[String]]
      if (null != dialcodes) {
        cdata :+= Map[String, AnyRef]("id" -> dialcodes, "type" -> "DialCode")
      }
    }
    if (null != addedRelations && addedRelations.nonEmpty)
      cdata ++= prepareCMap(addedRelations)
    if (null != removedRelations && removedRelations.nonEmpty)
      cdata ++= prepareCMap(removedRelations)
    cdata
  }

  /** @param relations
    */
  private def prepareCMap(
      relations: List[Map[String, AnyRef]]
  ): List[Map[String, AnyRef]] = {
  ...
  /** @param relations
    */
  private def getRelationProps(
      relations: List[Map[String, AnyRef]],
      objectDefinition: ObjectDefinition
  )(implicit config: TransactionEventProcessorConfig): List[String] = {
    ...
    if (relations.nonEmpty) {
      relations.foreach(rel => {
        ...
        val relationProp =
          objectDefinition.relationLabel(targetObjType, direction, relationType)
  ...
  @throws(classOf[InvalidEventException])
  def processAuditHistoryEvent(event: Event, metrics: Metrics)(implicit
    ...
    if (event.transactionEventProcessorIsValid) {
      ...
      try {
        ...
        case ex: IOException =>
          logger.error(
            "Error while indexing message :: " + event.getJson + " :: " + ex.getMessage
          )
          ex.printStackTrace()
        ...
        case ex: Exception =>
          logger.error(
            "Error while processing message :: " + event.getJson + " :: ",
            ex
          )
  ...
  private def getIndexName(
      ets: Long
  )(implicit config: TransactionEventProcessorConfig): String = {
    val cal = Calendar.getInstance(TimeZone.getTimeZone(config.timeZone))
    cal.setTime(new Date(ets))
    config.auditHistoryIndex + "_" + cal.get(Calendar.YEAR) + "_" + cal.get(
  ...
  def getAuditHistory(transactionDataMap: Event): AuditHistoryRecord = {
    val nodeUniqueId =
      StringUtils.replace(transactionDataMap.nodeUniqueId, ".img", "")
    AuditHistoryRecord(
      nodeUniqueId,
      transactionDataMap.objectType,
      transactionDataMap.label,
      transactionDataMap.graphId,
      transactionDataMap.userId,
      transactionDataMap.requestId,
      JSONUtil.serialize(transactionDataMap.transactionData),
      transactionDataMap.operationType,
      transactionDataMap.createdOnDate
    )
  }
}
