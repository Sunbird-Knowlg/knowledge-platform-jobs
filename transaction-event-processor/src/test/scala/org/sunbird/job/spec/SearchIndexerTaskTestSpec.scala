...
class SearchIndexerTaskTestSpec extends BaseTestSpec {
  ...
  val config: Config = ConfigFactory.load("search_indexer_test.conf")
  val jobConfig = new TransactionEventProcessorConfig(config)
  val mockElasticUtil =
  ...
  def setupElasticContainer(): Unit = {
    ...
    elasticContainer.withEnv(
      jobConfig.xpackSecurityKey,
      jobConfig.xpackSecurityEnabled
    )
  ...
  it should "return the indexable document for the provided object" in {
    ...
    val message =
      getEvent(EventFixture.DATA_NODE_CREATE, 509674).getMap().asScala.toMap
    ...
    val trackable = indexDocument
      .getOrElse("trackable", Map[String, AnyRef]())
      .asInstanceOf[Map[String, AnyRef]]
    indexDocument.isEmpty should be(false)
    indexDocument.getOrElse("identifier", "").asInstanceOf[String] should be(
      "do_1132247274257203201191"
    )
    trackable.getOrElse("enabled", "") should be("No")
    indexDocument.getOrElse("objectType", "").asInstanceOf[String] should be(
      "Collection"
    )
  }
  ...
  it should "return the indexable document with the added relation for the provided object" in {
    val definition =
      defCache.getDefinition("Collection", "1.0", jobConfig.definitionBasePath)
    val compositeFunc = new CompositeSearchIndexerFunction(jobConfig)
    val message = getEvent(EventFixture.DATA_NODE_CREATE_WITH_RELATION, 509674)
      .getMap()
      .asScala
      .toMap
    val indexDocument: Map[String, AnyRef] = compositeFunc.getIndexDocument(
    ...
    indexDocument.isEmpty should be(false)
    indexDocument.getOrElse("identifier", "").asInstanceOf[String] should be(
      "do_112276071067320320114"
    )
    indexDocument.getOrElse("objectType", "").asInstanceOf[String] should be(
      "Content"
    )
    indexDocument
      .getOrElse("collections", List[String]())
      .asInstanceOf[List[String]] should contain("do_1123032073439723521148")
  }

  it should "return the indexable document without the deleted relation for the provided object" in {
    val documentJson =
      """{"identifier":"do_112276071067320320114","graph_id":"domain","node_id":105631,"collections":["do_1123032073439723521148", "do_1123032073439723521149"],"objectType":"Content","nodeType":"DATA_NODE"}"""
    when(mockElasticUtil.getDocumentAsString(anyString()))
      .thenReturn(documentJson)

    val definition =
      defCache.getDefinition("Collection", "1.0", jobConfig.definitionBasePath)
    val compositeFunc = new CompositeSearchIndexerFunction(jobConfig)
    val message = getEvent(EventFixture.DATA_NODE_UPDATE_WITH_RELATION, 509674)
      .getMap()
      .asScala
      .toMap
    val indexDocument: Map[String, AnyRef] = compositeFunc.getIndexDocument(
  ...
  it should "return the indexable document for the provided update object" in {
    ...
    val compositeFunc = new CompositeSearchIndexerFunction(jobConfig)
    val message =
      getEvent(EventFixture.DATA_NODE_UPDATE, 509674).getMap().asScala.toMap
    val indexDocument: Map[String, AnyRef] = compositeFunc.getIndexDocument(
      message,
      true,
      definition,
    ...
    indexDocument.getOrElse("description", "").asInstanceOf[String] should be(
      "updated description"
    )
  }
  ...
  it should " give the document for indexing the DIAL Code metrics " in {
    val documentJson =
      """{"last_scan":1541456052000,"dial_code":"QR1234","first_scan":1540469152000,"total_dial_scans_local":25,"objectType":"","average_scans_per_day":2}"""
    when(mockElasticUtil.getDocumentAsString(anyString()))
      .thenReturn(documentJson)
    ...
    val response = dialcodeMetricFunc.getIndexDocument(
      event.getMap().asScala.toMap,
      true
    )(mockElasticUtil)
    response.isEmpty should be(false)
    response
      .getOrElse("total_dial_scans_global", 0)
      .asInstanceOf[Integer] should be(25)
  }
  ...
  it should " give the document for indexing the dialcode external " in {
    ...
    val response = dialcodeExternalFunc.getDocument(
      event.getMap().asScala.toMap,
      false
    )(mockElasticUtil)
    response.isEmpty should be(false)
    response.getOrElse("identifier", "").asInstanceOf[String] should be(
      "X8R3W4"
    )
    response.getOrElse("objectType", "").asInstanceOf[String] should be(
      "DialCode"
    )
    response.getOrElse("batchcode", "").asInstanceOf[String] should be(
      "testPub0001.20210212T011555"
    )
  }
...
//  }
  ...
  it should " index the DIAL Code metrics " in {
    ...
    dialcodeMetricFunc.upsertDialcodeMetricDocument(
      uniqueId,
      event.getMap().asScala.toMap
    )(mockElasticUtil)
  ...
  "upsertExternalDocument " should " index the dialcode external " in {
    ...
    val dialcodeExternalFunc = new DIALCodeIndexerFunction(jobConfig)
    val uniqueId = event.readOrDefault("nodeUniqueId", "")
    dialcodeExternalFunc.upsertDIALCodeDocument(
      uniqueId,
      event.getMap().asScala.toMap
    )(mockElasticUtil)
  ...
  "upsertExternalDocument " should " update and index the dialcode external " in {
    ...
    doNothing().when(mockElasticUtil).addDocument(anyString(), anyString())
    val documentJson =
      """{"channel":"channelTest","generated_on":"2021-02-12T01:16:07.750+0530","identifier":"X8R3W4","dialcode_index":9071809.0,"batchcode":"testPub0001.20210212T011555","objectType":"DialCode","status":"Draft","publisher":"testPub0001"}"""
    when(mockElasticUtil.getDocumentAsString(anyString()))
      .thenReturn(documentJson)
  ...
  "createCompositeSearchIndex" should "create the elastic search index for composite Search" in {
    Mockito.reset(mockElasticUtil)
    when(mockElasticUtil.addIndex(anyString(), anyString(), anyString()))
      .thenReturn(false)
    val compositeFunc = new CompositeSearchIndexerFunction(jobConfig)
  ...
  "DialCodeMetricIndexerFunction" should "return the event with error message" in {
    ...
    val failedEventString =
      compositeFunc.getFailedEvent(event.jobName, event.getMap(), exception)
  ...
  "Composite Search Indexer" should " sync the Data Node " in {
    ...
    val elasticUtil = new ElasticSearchUtil(
      ...
      jobConfig.compositeSearchIndex
    )
    ...
    BaseMetricsReporter
      .gaugeMetrics(
        s"${jobConfig.jobName}.${jobConfig.successCompositeSearchEventCount}"
      )
      .getValue() should be(1)
    BaseMetricsReporter
      .gaugeMetrics(
        s"${jobConfig.jobName}.${jobConfig.compositeSearchEventCount}"
      )
      .getValue() should be(1)
    BaseMetricsReporter
      .gaugeMetrics(
        s"${jobConfig.jobName}.${jobConfig.failedCompositeSearchEventCount}"
      )
  ...
  "Composite Search Indexer" should " sync the External Dialcode Data " in {
    ...
    val elasticUtil = new ElasticSearchUtil(
      jobConfig.esConnectionInfo,
      jobConfig.dialcodeExternalIndex
    )
    ...
    BaseMetricsReporter
      .gaugeMetrics(
        s"${jobConfig.jobName}.${jobConfig.successDialcodeExternalEventCount}"
      )
      .getValue() should be(1)
    BaseMetricsReporter
      .gaugeMetrics(
        s"${jobConfig.jobName}.${jobConfig.dialcodeExternalEventCount}"
      )
      .getValue() should be(1)
    BaseMetricsReporter
      .gaugeMetrics(
        s"${jobConfig.jobName}.${jobConfig.failedDialcodeExternalEventCount}"
      )
      .getValue() should be(0)
  }
  ...
  "Composite Search Indexer" should " sync the Dialcode Metrics Data " in {
    ...
    BaseMetricsReporter
      .gaugeMetrics(
        s"${jobConfig.jobName}.${jobConfig.successDialcodeMetricEventCount}"
      )
      .getValue() should be(1)
    BaseMetricsReporter
      .gaugeMetrics(
        s"${jobConfig.jobName}.${jobConfig.dialcodeMetricEventCount}"
      )
      .getValue() should be(1)
    BaseMetricsReporter
      .gaugeMetrics(
        s"${jobConfig.jobName}.${jobConfig.failedDialcodeMetricEventCount}"
      )
      .getValue() should be(0)
  }
...
private class CompositeSearchEventSource(events: List[String])
