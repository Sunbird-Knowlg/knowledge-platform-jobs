...
import org.sunbird.incredible.pojos.valuator.ExpiryDateValidator

class ExpiryDateValidatorTest  extends BaseTestSpec {
  ...
  "check expiry Date Is correct For Months" should "should return valid expiry date for months" in {
    val expiryDate = "2m"
    val expiryDateValidator = new ExpiryDateValidator(issuedDate)
    val expiry = expiryDateValidator.getExpiryDate(expiryDate)
