...
class PostPublishProcessorTaskTestSpec extends BaseTestSpec {
  ...
  "Post Publish Processor" should "process and find shallow copied contents" in {
    ...
    val list = new PostPublishEventRouter(jobConfig, mockHttpUtil, mockNeo4JUtil).getShallowCopiedContents(identifier)
  ...
  ignore should "run all the scenarios for a given event" in {
    ...
    val streamTask = new PostPublishProcessorStreamTask(jobConfig, mockKafkaUtil, mockHttpUtil, mockNeo4JUtil)
    streamTask.process()
