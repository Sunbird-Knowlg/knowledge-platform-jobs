    ...
    try {
      ...
      certTemplates.keySet().forEach(templateId => {
        ...
        usersToIssue.foreach(user => {
          ...
          println("final event send to next topic : " + gson.toJson(certEvent.toString))
          context.output(config.generateCertificateOutputTag, gson.toJson(certEvent.toString))
