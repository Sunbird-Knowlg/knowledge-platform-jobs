...
import org.sunbird.job.functions.{CertificateApiService, CertificateEventGenerator, CertificatePreProcessor}
...
import org.sunbird.job.domain.{CertTemplate, CertificateGenerateEvent}
...
class CertificatePreProcessorSpec extends BaseTestSpec {
  ...
  it should "fetchCertTemplates" in {
    ...
    val validTemplate:util.Map[String, AnyRef] = new CertificatePreProcessor(jobConfig)(stringTypeInfo, cassandraUtil) invokePrivate fetchCertTemplates(edata, metrics)
  ...
  it should "generateCertificateEvent" in {
    val dataCache = new DataCache(jobConfig, redisConnect, 2, List())
    dataCache.init()
    mockAll()
    val templateMap = gson.fromJson(EventFixture.USER_UTIL_EDATA, new util.LinkedHashMap[String, AnyRef]().getClass).asInstanceOf[util.Map[String, AnyRef]]
    val template = gson.fromJson(templateMap.toString, classOf[CertTemplate])
    val edata = gson.fromJson(EventFixture.USER_UTIL_EDATA, new util.LinkedHashMap[String, AnyRef]().getClass).asInstanceOf[util.Map[String, AnyRef]]
//    val generateCertificateEvent = PrivateMethod[CertificateGenerateEvent]('generateCertificateEvent)
//    val request:CertificateGenerateEvent = new CertificatePreProcessor(jobConfig)(stringTypeInfo, cassandraUtil) invokePrivate generateCertificateEvent("user_001", template, edata, dataCache, metrics)
//    request.actor.`id` should equal("Certificate Generator")
    dataCache.close()
  }
...
//    val certificateEventGenerator = mockObject(new CertificateEventGenerator(jobConfig)(metrics, cassandraUtil))
//    val edata = gson.fromJson(EventFixture.FINAL_EDATA, new util.LinkedHashMap[String, AnyRef]().getClass).asInstanceOf[util.Map[String, AnyRef]]
//    when(certificateEventGenerator.prepareGenerateEventEdata(any(classOf[util.Map[String, AnyRef]]), any(classOf[DataCache]))).thenReturn(edata)
  }
