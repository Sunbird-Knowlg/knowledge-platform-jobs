...
class AssetEnrichmentConfig(override val config: Config) extends BaseJobConfig(config, "asset-enrichment") {
  ...
  val definitionBasePath: String = if (config.hasPath("schema.base_path")) config.getString("schema.base_path") else "https://raw.githubusercontent.com/Sunbird-Knowlg/knowledge-platform/master/schemas/"
