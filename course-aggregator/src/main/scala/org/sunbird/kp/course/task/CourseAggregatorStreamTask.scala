...
class CourseAggregatorStreamTask(config: CourseAggregatorConfig, kafkaConnector: FlinkKafkaConnector) {
  ...
  def process(): Unit = {
     ...
     */
    val deDupStream =
      env.addSource(kafkaConnector.kafkaMapSource(config.kafkaInputTopic), config.courseAggregatorConsumer)
        .uid(config.courseAggregatorConsumer).setParallelism(config.kafkaConsumerParallelism)
        .rebalance()
