# Course Aggregate Updater

Course agg job is used to compute the progress for unit level and course level for each batch and user and updates to database.
...
```
./bin/flink run -m yarn-cluster -p 2 -yjm 1024m -ytm 1024m <knowledge-platform-jobs>/course-aggregate-updater/target/course-aggregate-updater-0.0.1.jar
```
...
# Submit the course-metrics-agg job to the Kubernetes single node cluster flink-cluster
./bin/flink run -m localhost:8081 <knowledge-platform-job>/course-aggregate-updater/target/ourse-metrics-aggregator-0.0.1.jar
