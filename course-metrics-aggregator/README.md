# Course Metrics Aggregator

Course metrics agg job is used to compute the progress for unit level and course level for each batch and user.
...
```
./bin/flink run -m yarn-cluster -p 2 -yjm 1024m -ytm 1024m <knowledge-platform-jobs>/course-metrics-aggregator/target/course-metrics-aggregator-0.0.1.jar
```
...
./bin/flink run -m localhost:8081 <knowledge-platform-job>/course-metrics-aggregator/target/ourse-metrics-aggregator-0.0.1.jar
