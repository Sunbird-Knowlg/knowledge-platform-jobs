...
import org.sunbird.job.functions.QuestionSetPublisher
import org.sunbird.job.publish.domain.{Event, PublishMetadata}
import org.sunbird.job.util.{FlinkUtil, HttpUtil}

class QuestionSetPublishPipelineTask(config: QuestionSetPublishPipelineConfig, kafkaConnector: FlinkKafkaConnector, httpUtil: HttpUtil) {
...
object QuestionSetPublishPipelineTask {
	...
	def main(args: Array[String]): Unit = {
		...
		val task = new QuestionSetPublishPipelineTask(publishConfig, kafkaUtil, httpUtil)
		task.process()
	}
}
